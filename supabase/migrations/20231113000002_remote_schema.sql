...
CREATE POLICY "查看自己宠物" ON public.pets FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "插入自己宠物" ON public.pets FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "更新自己宠物" ON public.pets FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "删除自己宠物" ON public.pets FOR DELETE USING (auth.uid() = user_id);
...
CREATE POLICY "查看点赞" ON public.likes FOR SELECT USING (true);
...
CREATE POLICY "读评论" ON public.comments FOR SELECT USING (true);
CREATE POLICY "查看自己评论" ON public.comments FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "插入自己评论" ON public.comments FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "更新自己评论" ON public.comments FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "删除自己评论" ON public.comments FOR DELETE USING (auth.uid() = user_id);
...
CREATE POLICY "看自己收藏" ON public.favorites FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "看自己收藏" ON public.favorites FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "插入自己收藏" ON public.favorites FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "删除自己收藏" ON public.favorites FOR DELETE USING (auth.uid() = user_id);
...
-- 徽章仅用于查询，写入由触发器/函数执行，无需用户级 RLS
-- 若需手动颁发，可临时使用 service_role
...
CREATE POLICY "看自己通知" ON public.notifications FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "看自己通知" ON public.notifications FOR SELECT USING (auth.uid() = user_id);
