...
class _ForgotPasswordPageState extends State<ForgotPasswordPage> {
  ...
  Future<void> _resetPassword() async {
    ...
    try {
      ...
      // 验证码通过后，重置密码
      final resetResult = await ApiService().resetPassword(phone, pwd);
      if (!mounted) return;

      if (resetResult['code'] == 200) {
        Toast.success(resetResult['message'] ?? '密码重置成功');
        Navigator.pushReplacementNamed(context, '/login');
      } else {
        Toast.error(resetResult['message'] ?? '密码重置失败');
      }
    } catch (e) {
      Toast.error('重置失败：$e');
    } finally {
                      ...
                      child: Column(
                        children: [
                          ...
                          Container(
                            ...
                            child: Center(
                              child: Icon(Icons.lock_reset_rounded, size: 40.w, color: Colors.white),
                            ),
                          ),
